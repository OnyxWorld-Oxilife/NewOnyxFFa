...
public class WantedEvent {
    ...
    public void setTarget(FFaPlayer newTarget) {
        target = newTarget;
        Bukkit.broadcastMessage("§e[§rWanted§e] La nouvelle cible est §r" + target.getPlayer().getName() + "§e!")  ;
        target.getPlayer().addPotionEffect(new PotionEffect(PotionEffectType.DAMAGE_RESISTANCE, 2 , 9999, true));
        target.getPlayer().addPotionEffect(new PotionEffect(PotionEffectType.GLOWING, 2 , 9999, true));
        target.getPlayer().addPotionEffect(new PotionEffect(PotionEffectType.SLOW, 1 , 9999, true));
    }
